...
const APP='./app-v2.html?v=7';
const APP_FALLBACK='./app-v2.html';
const ASSETS=[APP_FALLBACK,'./workout-points.js','./wellbeing-history.js','./reminder.js','./manifest.webmanifest'];
...
async function patchedApp(response){
 ...
 let patched=text;
 if(!patched.includes('workout-points.js')) patched=patched.replace('</body>','<script src="./workout-points.js?v=7"></script></body>');
 if(!patched.includes('wellbeing-history.js')) patched=patched.replace('</body>','<script src="./wellbeing-history.js?v=7"></script></body>');
 if(!patched.includes('reminder.js')) patched=patched.replace('</body>','<script src="./reminder.js?v=7"></script></body>');
